...
import voucherFakerFactory from "./voucherFakerFactory";
...
const VoucherFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [categoryidIds, setcategoryidIds] = useState();
    ...
    const onRun = async () => {
        let fakeData = voucherFakerFactory(props.user,fakerCount,categoryidIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("voucher").create(elem));
...
export default connect(mapState, mapDispatch)(VoucherFakerDialogComponent);
