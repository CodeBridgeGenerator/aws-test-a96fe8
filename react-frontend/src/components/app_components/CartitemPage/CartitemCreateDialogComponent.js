...
const CartitemCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            id: _entity?.id?._id,id: _entity?.id,voucherid: _entity?.voucherid,userid: _entity?.userid,quantity: _entity?.quantity,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Cartitem created successfully" });
    ...
    return (
        <Dialog header="Create Cartitem" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            ...
            <span className="align-items-center">
                ...
                <Dropdown id="id" value={_entity?.id?._id} optionLabel="name" optionValue="value" options={~options~} onChange={(e) => setValByKey("id", {_id : e.value})}  />
            </span>
